...
3.  **Router:** Create `app/router.py` to parse URLs, extract the `board_token`, and trigger the right extractor.
...
The root `main.py` command is intentionally a thin wrapper. Backend packages now
live under `app/`: command implementations in `app/cli/` and `app/phases/`,
shared extraction in `app/extractors/`, normalization in `app/normalizers/`,
storage in `app/db/`, API routes in `app/api/`, and seed lists in
`app/sources/`.
