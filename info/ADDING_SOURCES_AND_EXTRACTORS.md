...
5. Import:
   ...
   ```powershell
   python -m db.import_phase3 outputs/new_normalized.json
   ```
...
4. Add the provider name to `AtsProvider` in `models.py`.

5. Export the extractor in `extractors/__init__.py` if it needs package-level
   access.

6. Register routing in `router.py`:
...
- Router detection in `tests/test_phase2.py` or a provider-specific test file.
