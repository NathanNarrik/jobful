...
```text
Celery Beat or python -m app.phases.phase2
  -> Redis queue
  -> Celery worker
  -> app.cli.extract.extract_single_url(...)
  -> AtsRouter
  -> extractor
...
same extraction path. The root `main.py` file is a thin Phase 1 wrapper; phase
commands run as modules under `app.phases`.

## Package Layout

```text
app/api/      FastAPI app, schemas, and read-only routes
app/cli/      Command implementations for extraction, enqueueing, normalization, and audits
app/db/       SQLAlchemy models, sessions, import, stale-job maintenance
app/extractors/ ATS-specific extraction strategies and shared extractor helpers
frontend/     Next.js discovery feed and Kanban tracking UI
app/normalizers/ Phase 3 cleaning, heuristics, Ollama, and normalization pipeline
app/scripts/  One-off discovery/debugging helpers
app/tests/    Unit and API tests
app/sources/  Default seed URLs and source-list text files
info/         Long-form project notes
```

## Main Entry Points

### `main.py` / `app.cli.extract`
...
python main.py --input-file app/sources/sources_user_requested_companies_expanded.txt --workers 12 --timeout 8
```
...
### `python -m app.phases.phase2` / `app.cli.enqueue`
...
### `python -m app.phases.phase3` / `app.cli.normalize`
...
```powershell
python -m app.phases.phase3 outputs/full_default_confirmation.json --limit 500 --no-ollama
...
All extractors live in `app/extractors/`.
...
- `app/extractors/amazon.py`
- `app/extractors/apple.py`
- `app/extractors/google.py`
- `app/extractors/oracle.py`
- `app/extractors/talentbrew.py`
- `app/extractors/avature.py`
...
- `app/sources/sources_fortune_tech.txt`
- `app/sources/sources_fortune_plus_quant.txt`
- `app/sources/sources_finance_quant_expansion.txt`
- `app/sources/sources_top_companies_expanded_candidates.txt`
- `app/sources/sources_user_requested_company_targets.txt`
- `app/sources/sources_user_requested_companies_expanded.txt`

Use them like:

```powershell
python main.py --input-file app/sources/sources_user_requested_companies_expanded.txt
python -m app.phases.phase2 --input-file app/sources/sources_user_requested_companies_expanded.txt
```
...
Phase 3 lives in `app/normalizers/` plus the `python -m app.phases.phase3` CLI.

### `app/normalizers/cleaner.py`
...
against `mistral:latest` through `python -m app.phases.phase3_ollama_check` and
...
The `app/scripts/` directory contains discovery and maintenance utilities. These
...
- stale-job marking with `python -m app.db.mark_stale`
...
See `info/PHASE5_README.md` for the Phase 5 contract and commands.
